...
// modified 8/2/08 by JCF (fiala@bu.edu)
// -+- change: Add remembered location for ObjectAttributes, ObjectRename, ListObjects, ListZtraces, ListDistances
//
...
{
	static RECT loc = {0,0,0,0};						// remember position and size of window
	CHOOSECOLOR cc;
	Contour *c;
	int i;
	char txt[64];
	...
	switch (message)					// enter with CurrContour set from first active contour so can set values after OK
		...
		case WM_INITDIALOG:
			if ( loc.bottom )
				MoveWindow(hwndDlg,loc.left,loc.top,loc.right-loc.left,loc.bottom-loc.top,FALSE);
			SendDlgItemMessage(hwndDlg, ID_CONTOURNAME, WM_SETTEXT, 0, (LPARAM)InputDlgName);
			FillDlgObject( hwndDlg );
			return TRUE;

		case WM_MOVE:
			GetWindowRect( hwndDlg, &loc );		// remember the new position
			break;

		case WM_SIZE:
			GetWindowRect( hwndDlg, &loc );		// remember the new size
			break;
...
LRESULT APIENTRY ListObjectsProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
		...
		case WM_CREATE:
			if ( loc.bottom )
				MoveWindow(hWnd,loc.left,loc.top,loc.right-loc.left,loc.bottom-loc.top,FALSE);
		...
		case WM_MOVE:
			GetWindowRect( hWnd, &loc );		// remember the new position
			break;

		case WM_SIZE:
			GetWindowRect( hWnd, &loc );		// remember the new size
			GetClientRect( hWnd, &r );			// resize list view control to match dialog box
...
void InterpolateZTraces( HWND list )		// add points to intervening sections along each selected z-trace
{
	Contours *zcontour_list;
	...
	if ( zcontour_list->Number() && CurrSectionsInfo )	// found some z-traces, so do it...
		{
		zcontour = zcontour_list->first;				// interpolate each z-trace
		while ( zcontour )
			{
			ncontour = new Contour( *zcontour );		// copy the ztrace and name
			strcpy(ncontour->name,"Intrp. of ");
			strncat(ncontour->name,zcontour->name,MAX_CONTOUR_NAME-strlen(ncontour->name));
			CurrSeries->zcontours->Add( ncontour );		// add it to the series zTraces

			p = ncontour->points->first;				// check every pt pair of new z-trace
			if ( p ) q = p->next;
			while ( p && q )
				{
				first_sect = (int)p->z;					// z values are section numbers
				last_sect = (int)q->z;
				for (sectnum=first_sect+1; sectnum<last_sect; sectnum++)
				  if ( CurrSectionsInfo->IsSection(sectnum) )
					{
					zp = CurrSectionsInfo->ZDistance( (int)p->z, CurrSeries->zMidSection );
					zq = CurrSectionsInfo->ZDistance( (int)q->z, CurrSeries->zMidSection );
					zn = CurrSectionsInfo->ZDistance( sectnum, CurrSeries->zMidSection );
					if ( zq != zp ) s = (zn-zp)/(zq-zp);
					else s = 0.0;
					x = p->x*(1-s)+s*q->x;
					y = p->y*(1-s)+s*q->y;
					n = new Point( x, y, (double)sectnum );
					ncontour->points->Insert( n, p );
					p = n;
					}
				p = q;
				q = q->next;
	...
	switch (message)
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
				...
				case CM_INTERPOLATEZTRACES:
					InterpolateZTraces( zTraceList );
					break;
				}
		}
	return( DefWindowProc(hWnd, message, wParam, lParam) );
